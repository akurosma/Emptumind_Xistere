...
static struct ObjectHitbox sRlTripletButterflyExplodeHitbox = {
...
static struct RlTripletButterflyActivationData sRlTripletButterflyActivationData[] = {
    { MODEL_BOWLING_BALL, NULL, 0.5f },
    //{ MODEL_BOWLING_BALL, NULL, 1.0f },
    /*{ MODEL_1UP, bhv1upWalking, 1.0f },*/
};

static void rl_tripletbutterfly_act_init(void) {
    s32 butterflyNum = o->oBehParams2ndByte & RL_TRIPLETBUTTERFLY_BP_BUTTERFLY_NUM;
    s32 i;

    if (butterflyNum != 0 || o->oDistanceToMario < 1400.0f) {
        if (butterflyNum == 0) {
            for (i = 1; i <= 2; i++) {
        ...
        /*if (o->parentObj->oTripletButterflySelectedButterfly == o->oBehParams2ndByte) {
            o->oTripletButterflyType = RL_TRIPLETBUTTERFLY_TYPE_SPAWN_1UP;
        } else if (o->parentObj->oBehParams2ndByte & RL_TRIPLETBUTTERFLY_BP_NO_BOMBS) {
            o->oTripletButterflyType = RL_TRIPLETBUTTERFLY_TYPE_NORMAL;
        }*/
        // Default butterfly type is TRIPLET_BUTTERFLY_TYPE_EXPLODES

        o->oAction = RL_TRIPLETBUTTERFLY_ACT_WANDER;
...
static void rl_tripletbutterfly_act_wander(void) {
    if (o->oDistanceToMario > 1500.0f) {
        ...
        if (o->oTimer < 60) {
            ...
            if (o->oTimer > 55 && o->oDistanceToMario < 1400.0f
                && o->oTripletButterflyType > RL_TRIPLETBUTTERFLY_TYPE_NORMAL) {
                o->oAction = RL_TRIPLETBUTTERFLY_ACT_ACTIVATE;
...
static void rl_tripletbutterfly_act_activate(void) {
    if (o->oTimer > 20) {
        if (o->oTripletButterflyModel == 0) {
            spawn_object_relative_with_scale(0, 0, -40, 0, 1.5f, o, MODEL_SMOKE, bhvWhitePuffSmoke2);
            o->oTripletButterflyModel = sRlTripletButterflyActivationData[o->oTripletButterflyType].model;
            cur_obj_set_model(o->oTripletButterflyModel);
            obj_set_billboard(o);
            o->oTripletButterflyScale = 0.0f;
            o->oHomeY = o->oPosY;
        } else if (o->oTripletButterflyScale
                   >= sRlTripletButterflyActivationData[o->oTripletButterflyType].scale) {
            if (o->oTripletButterflyType != RL_TRIPLETBUTTERFLY_TYPE_EXPLODES) {
                spawn_object(o, o->oTripletButterflyModel,
                             sRlTripletButterflyActivationData[o->oTripletButterflyType].behavior);
                obj_mark_for_deletion(o);
            } else {
                o->oAction = RL_TRIPLETBUTTERFLY_ACT_EXPLODE;
                o->oWallHitboxRadius = 100.0f;
            }
        }

        o->oTripletButterflyScale +=
            sRlTripletButterflyActivationData[o->oTripletButterflyType].scale / 30.0f;
        if (o->oTripletButterflyType == RL_TRIPLETBUTTERFLY_TYPE_EXPLODES) {
...
static void rl_tripletbutterfly_act_explode(void) {
    obj_check_attacks(&sRlTripletButterflyExplodeHitbox, -1);
...
void bhv_rl_tripletflame_update(void) {
    ...
    switch (o->oAction) {
        case RL_TRIPLETBUTTERFLY_ACT_INIT:
            rl_tripletbutterfly_act_init();
            break;
        case RL_TRIPLETBUTTERFLY_ACT_WANDER:
            rl_tripletbutterfly_act_wander();
            break;
        case RL_TRIPLETBUTTERFLY_ACT_ACTIVATE:
            rl_tripletbutterfly_act_activate();
            break;
        case RL_TRIPLETBUTTERFLY_ACT_EXPLODE:
            rl_tripletbutterfly_act_explode();
            break;
    }
